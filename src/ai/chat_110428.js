    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (!response.ok) {
                ...
                logger.error(`OpenRouter API error (attempt ${attempt}): ${response.status} — ${errorBody}`);
                ...
                if (attempt === MAX_RETRIES) throw new Error(`API returned ${response.status}`);
                continue;
            }
            ...
            if (data.choices && data.choices.length > 0 && data.choices[0].message) {
                ...
                if (attempt === MAX_RETRIES) throw new Error('No valid response from AI');
            }

        } catch (error) {
            if (error.name === 'AbortError') {
                logger.error(`AI request timeout (attempt ${attempt})`);
            } else {
                logger.error(`AI request failed (attempt ${attempt}):`, error.message);
            }
